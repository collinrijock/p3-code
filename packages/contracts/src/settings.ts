...
export const ClaudeSettings = makeProviderSettingsSchema(
  {
    ...
    loadUserPlugins: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({
        title: "Load user-installed plugins",
        description:
          "Allows enabled Claude Code plugins to run their skills, MCP servers, and hooks in new sessions. Only enable plugins you trust.",
  ...
  {
    order: ["binaryPath", "homePath", "loadUserPlugins", "autoCompactWindow", "launchArgs"],
  },
);
...
export const PhiSettings = makeProviderSettingsSchema(
  {
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ...
    binaryPath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Phi binary. Leave blank to try `phi`, then `pi`.",
        providerSettingsForm: {
          placeholder: "Automatic (phi, then pi)",
          clearWhenEmpty: "omit",
        },
      }),
    ),
    homePath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        description: "Custom Phi/Pi agent config directory.",
        providerSettingsForm: {
...
export type PhiSettings = typeof PhiSettings.Type;
...
export const ServerSettings = Schema.Struct({
  ...
  providers: Schema.Struct({
    ...
    phi: PhiSettings.pipe(Schema.withDecodingDefault(Effect.succeed({}))),
  }).pipe(Schema.withDecodingDefault(Effect.succeed({}))),
...
const ClaudeSettingsPatch = Schema.Struct({
  ...
  binaryPath: Schema.optionalKey(TrimmedString),
  homePath: Schema.optionalKey(TrimmedString),
  loadUserPlugins: Schema.optionalKey(Schema.Boolean),
  customModels: Schema.optionalKey(Schema.Array(Schema.String)),
  launchArgs: Schema.optionalKey(TrimmedString),
...
const PhiSettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  binaryPath: Schema.optionalKey(TrimmedString),
  homePath: Schema.optionalKey(TrimmedString),
});
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  providers: Schema.optionalKey(
    Schema.Struct({
      ...
      phi: Schema.optionalKey(PhiSettingsPatch),
    }),
  ),
