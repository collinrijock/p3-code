 ...
 *     (`codex`, `claudeAgent`, `cursor`, `grok`, `opencode`, `phi`) in a single
 ...
 * without trying to spawn real `codex` / `claude` / `agent` / `grok` / `opencode` / `phi`
...
import { PhiDriver } from "../Drivers/PhiDriver.ts";
...
const makePhiConfig = (overrides: Partial<PhiSettings>): PhiSettings => ({
  enabled: false,
  binaryPath: "",
  homePath: "",
  ...overrides,
...
describe("ProviderInstanceRegistryLive — all drivers slice", () => {
  ...
  it.live("boots one instance of every shipped driver from a single config map", () =>
    Effect.gen(function* () {
      ...
      const phiId = ProviderInstanceId.make("phi_default");
      ...
      const phiDriverKind = ProviderDriverKind.make("phi");
      ...
      const configMap: ProviderInstanceConfigMap = {
        ...
        [phiId]: {
          driver: phiDriverKind,
          displayName: "Phi",
          enabled: false,
          config: makePhiConfig({}),
        },
      };

      const { registry } = yield* makeProviderInstanceRegistry<BuiltInDriversEnv>({
        drivers: [CodexDriver, ClaudeDriver, CursorDriver, GrokDriver, OpenCodeDriver, PhiDriver],
        configMap,
      });
      ...
      expect(instances.map((instance) => instance.instanceId).toSorted()).toEqual(
        [codexId, claudeId, cursorId, grokId, openCodeId, phiId].toSorted(),
      );
      ...
      const phi = yield* registry.getInstance(phiId);
      expect(codex?.driverKind).toBe(codexDriverKind);
      expect(claude?.driverKind).toBe(claudeDriverKind);
      expect(cursor?.driverKind).toBe(cursorDriverKind);
      expect(grok?.driverKind).toBe(grokDriverKind);
      expect(openCode?.driverKind).toBe(openCodeDriverKind);
      expect(phi?.driverKind).toBe(phiDriverKind);
      expect(codex?.displayName).toBe("Codex");
      expect(claude?.displayName).toBe("Claude");
      expect(cursor?.displayName).toBe("Cursor");
      expect(grok?.displayName).toBe("Grok");
      expect(openCode?.displayName).toBe("OpenCode");
      expect(phi?.displayName).toBe("Phi");
      ...
      const phiSnapshot = yield* phi!.snapshot.getSnapshot;
      expect(phiSnapshot.instanceId).toBe(phiId);
      expect(phiSnapshot.driver).toBe(phiDriverKind);
      expect(phiSnapshot.enabled).toBe(false);
      expect(phiSnapshot.continuation?.groupKey).toBe(`${phiDriverKind}:instance:${phiId}`);
    }).pipe(Effect.provide(testLayer)),
  );
});
